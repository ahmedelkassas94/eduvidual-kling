...
def ai_plan_ingredients(prompt: str, target_duration_s: int = 15) -> ProjectState:
    """New architecture: 15s shot-by-shot script + named ingredients. Claude writes the main script (high detail) and T2I prompts; Claude also fills I2V prompts per shot."""
    ...
    env_loader.require_env("ANTHROPIC_API_KEY", "Script is written by Claude. Add ANTHROPIC_API_KEY to .env (https://console.anthropic.com/).")

    # I2V movement_prompt is left empty in the plan; it is generated later by the orchestrator when processing each shot.
